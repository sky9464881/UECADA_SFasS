  ...
  try {
    ...
    if (data?.length) {
      users.value = data.map(u => ({
        id: u.userId,
        name: u.userName,
        role: (u.roleName === 'ADMIN' || u.roleName === 'MANAGER') ? '관리자' : '작업자',
        department: u.email?.split('@')[0] ?? '-',
        status: '활성',
        lastLogin: u.createdAt ? u.createdAt.slice(0, 16).replace('T', ' ') : '-',
      }))
    }
  } catch (e) {
    console.warn('[UserManagement] API 연결 실패, 데모 데이터 표시:', e.message)
  }
})
...
const userStats = computed(() => {
  ...
  const admin = list.filter((u) => u.role === '관리자').length
  const operator = list.filter((u) => u.role === '작업자').length
  const locked = list.filter((u) => u.status === '잠금').length
