...
function severityLabel(s) {
  if (!s) return '정보'
  const m = { CRITICAL: '긴급', WARNING: '경고', INFO: '정보' }
  return m[s.toUpperCase()] ?? s
}
// 상태 → 한글
function statusLabel(s) {
  if (!s) return '미처리'
  return s === 'RESOLVED' ? '처리완료' : '조치중'
}
  ...
  try {
    ...
    if (displayRows.length) {
      ...
      alarmSummary.value = [
        ...
        { label: '처리 완료', value: resolved, detail: `완료율 ${total ? Math.round(resolved/total*100) : 0}%`, tone: 'done' },
      ...
      equipmentFrequency.value = eqSorted.map(([code, cnt]) => ({
        label: code, count: cnt, percent: Math.round(cnt / maxEq * 100), type: '',
      }))
      ...
      typeFrequency.value = typeSorted.map(([t, cnt]) => ({
        label: t, count: cnt, percent: Math.round(cnt / maxType * 100),
      }))
    }

    // 시간별 추이 (stats → date 기반이므로 daily로 표시)
    if (stats?.length) {
      const maxCount = Math.max(...stats.map(s => Number(s.count)), 1)
      trendHours.value = stats.slice(0, 8).map(s => ({
        time: s.date?.slice(-5) ?? '-',
        count: Number(s.count),
        percent: Math.round(Number(s.count) / maxCount * 100),
      }))
    }
  } catch (e) {
    console.warn('[AlarmPage] API 연결 실패, 데모 데이터 표시:', e.message)
  }
})
</script>
