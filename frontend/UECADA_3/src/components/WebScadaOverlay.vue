...
function preconnect(url: string): void {
  try {
    ...
    // buildSmwpOverlayUrl performs the configuration validation.
  }
}
...
.scada-dialog-iconbtn:hover:not(:disabled) {
  background: rgba(56, 189, 248, 0.16);
  color: #e0f2fe;
}

.scada-dialog-iconbtn:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}

.scada-dialog-iconbtn--close:hover:not(:disabled) {
...
.scada-dialog-spin {
  animation: scada-dialog-spin 1s linear infinite;
  color: #38bdf8;
}

@keyframes scada-dialog-spin {
  to {
...
.scada-dialog-fallback p {
  margin: 0;
  font-size: 13px;
  color: #94a3b8;
}

.scada-dialog-fallback svg {
  color: #38bdf8;
}

.scada-dialog-fallback h3 {
  margin: 0;
  font-size: 16px;
  color: #f8fafc;
}

.scada-dialog-fallback-actions {
  display: flex;
  flex-wrap: wrap;
  justify-content: center;
  gap: 8px;
