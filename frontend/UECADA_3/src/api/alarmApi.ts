...
function toSummary(list: AlarmResponse[]): AlarmSummaryItem[] {
  ...
  return [
    ...
    { label: '미처리 알람', value: pending, detail: 'OPEN·조치중 포함', tone: 'pending' },
  ]
}
...
export async function fetchAlarmsRaw(options: AlarmListOptions = {}): Promise<AlarmResponse[]> {
  if (useMockAlarms()) {
...
export async function fetchAlarmList(options: AlarmListOptions = {}): Promise<AlarmListResponse> {
  const data = await fetchAlarmsRaw(options)
