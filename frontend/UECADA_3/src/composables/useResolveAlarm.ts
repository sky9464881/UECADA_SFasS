...
const COMMUNITY_ALARM_KEY = ['community', 'alarm-list'] as const
...
export function useResolveAlarm() {
  ...
  return useMutation({
    ...
    onMutate: async ({ alarmId, payload }) => {
      ...
      await queryClient.cancelQueries({ queryKey: [...COMMUNITY_ALARM_KEY] })
    ...
    onSettled: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [...COMMUNITY_ALARM_KEY] })
    },
  })
}
