...
| 실시간 갱신 | SMWP 페이지의 OEE 라벨이 1초마다 변하는지. 안 변하면 백엔드에 `node scripts/demo-metrics-pusher.mjs` 한 번 돌려 데이터 주입 |
...
- (선택) 1초 단위로 외부에서 더미 KPI 를 주입하고 싶으면 UECADA 루트에서 `node scripts/demo-metrics-pusher.mjs` 실행 — `/api/lines` 응답이 sin 파형으로 진동.
