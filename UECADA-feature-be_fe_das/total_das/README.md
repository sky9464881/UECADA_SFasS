...
The wrapper validates the fixed Node-RED flow for each line and then runs Docker Compose with the matching `.env.lineNN`.
Docker Compose always mounts `equip-sim\nodered\flows_das_LINE-01.json`,
`flows_das_LINE-02.json`, or `flows_das_LINE-03.json` into the line Node-RED
container as `/data/flows.json`; it does not rebuild or overwrite these files
during `up`.
